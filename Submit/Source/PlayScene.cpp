...
PlayScene::PlayScene(GameObject* parent) :
	...
	hImgBG(-1),
	SHOW_TURN_IMAGE_POS_Y(275),
	SHOW_MODE_IMAGE_POS_Y(320),
...
{
	std::fill(hImgTurn, hImgTurn + CONTROL_MAX, -1);
	std::fill(hImgMode, hImgMode + MODE_MAX, -1);
...
void PlayScene::Initialize()
{
	...
	//現在ターンの画像
	Transform imgTra;
	imgTra.position_.y = SHOW_TURN_IMAGE_POS_Y;
	InitImage(hImgTurn[CONTROL_1P], "Game\\turn1.png", imgTra);
	InitImage(hImgTurn[CONTROL_2P], "Game\\turn2.png", imgTra);
	imgTra.position_.y = SHOW_MODE_IMAGE_POS_Y;
	InitImage(hImgMode[MODE_SET], "Game\\modeset.png", imgTra);
	InitImage(hImgMode[MODE_ROTATE], "Game\\moderotate.png", imgTra);
	InitImage(hImgMode[MODE_VIEW], "Game\\modefree.png", imgTra);
	//音声読込
	...
	titleButton->SetMode(ButtonEx::MODE::PUSH_UP);
	titleButton->EnAutoIdle(true);
	...
	restartButton->EnAutoIdle(true);

	//titleButton->AddLinkButton(restartButton);
	//titleButton->SetNextKey(ButtonEx::DIR::DIR_RIGHT, restartButton);
	//restartButton->AddLinkButton(titleButton);
	//restartButton->SetNextKey(ButtonEx::DIR::DIR_LEFT, titleButton);
			...
			case PlayScene::MODE_SET:
				...
				if (Input::IsKeyDown(GetKey(KEY::KEY_ACT))) {
					...
					if (cube[selectData.x][selectData.y][selectData.z]->GetMark(selectData.surface) == Cube::MARK_BLANK) {
						switch (control)
						{
						case PlayScene::CONTROL_1P:   cube[selectData.x][selectData.y][selectData.z]->SetMark(selectData.surface, Cube::MARK::MARK_O);  break;
						case PlayScene::CONTROL_2P:   cube[selectData.x][selectData.y][selectData.z]->SetMark(selectData.surface, Cube::MARK::MARK_X);  break;
						}
...
void PlayScene::Draw()
{
	...
	if (finished) {
		...
		Image::Draw(hImgTurn[control]);
		Image::Draw(hImgMode[mode]);
		endImage->Invisible();
	}
}
...
void PlayScene::ButtonAct(int hAct)
{
	switch (hAct)
	{
...
void PlayScene::GameInit()
{
	...
	isRotating = false;
	rotSpdX = 0;	rotSpdY = 0;
	camDirHN = CAM_FRONT;	camDirVT = CAM_MIDDLE;
	rotProgress = 0;
	finished = false;
	winRotProgress = 0;
	enFreeLook = false;
	...
	//カメラ位置設定
	camTra.SetPosition(0.0f,0.0f,-CAM_DISTANCE);
	camTra.rotate_ = { 0,0,0 };
	Camera::SetPosition(camTra.position_);
	...
	//ボタン非表示
	titleButton->Leave();
	titleButton->Invisible();
	restartButton->Leave();
	restartButton->Invisible();
}

void PlayScene::InitImage(int& handle, std::string file, Transform tra)
{
	handle = Image::Load(file);
	Image::SetTransform(handle, tra);
}

//============================ 選択関連 ============================

void PlayScene::MoveSelect(MODE mode)
{
	...
	case PlayScene::MODE_SET:
		...
		case SURFACE::SURFACE_TOP:
			switch (camDirHN)
			{
			case PlayScene::CAM_FRONT:
				break;
			case PlayScene::CAM_LEFT:
				//WASDをASDWに
				tmp = keys.front();
				keys.erase(keys.begin());
				keys.push_back(tmp);
				break;
			case PlayScene::CAM_RIGHT:
				//WASDをDWASに
				tmp = keys.back();
				keys.pop_back();
				keys.insert(keys.begin(), tmp);
				break;
			case PlayScene::CAM_BACK:
				//WASDをSDWAに
				for (int i = 0; i < 2; i++) {
					tmp = keys.back();
					keys.pop_back();
...
void PlayScene::MoveSelectParts(DIR dir, bool plus, Cube::SURFACE outSurface) {

	int* target = nullptr;
	switch (dir)
	{
	case PlayScene::X:target = &selectData.x; break;
	case PlayScene::Y:target = &selectData.y; break;
	case PlayScene::Z:target = &selectData.z; break;
	}
	if (target != nullptr) {
		if (plus) {
			*target += 1;
...
void PlayScene::MoveSelectChangeDir(ROTATE_DIR changeDir)
{
	selectData.dir = changeDir;
	indicator->SetCubeRotate(selectData.dir);
}

void PlayScene::MoveSelectSlideCol(bool isPlus)
{
	if (isPlus) {
...
void PlayScene::SetModeInit()
{
	...
	switch (camDirVT)
	{
	case PlayScene::CAM_TOP:
		selectData.y = PIECES - 1;
		break;
	case PlayScene::CAM_MIDDLE:
...
void PlayScene::UpdateCubeTransform()
{
...
void PlayScene::RotateCube(int prog, int maxProg, ROTATE_DIR dir) {
	switch (dir)
	{
...
void PlayScene::CompletedRotate()
{
	//キューブの親連結を解除
	//キューブ情報の入れ替え
	//キューブインスタンス自体を入れ替え
...
void PlayScene::SwapCubeModifySwapCount(int* swapCount, int row, bool isCC)
{
...
Cube::MARK PlayScene::CheckMarkVH(int x, int y, int z, SURFACE surface, DIR dir)
{
...
Cube::MARK PlayScene::CheckMarkVH(XMINT3 xyz, SURFACE surface, DIR dir)
{
	return CheckMarkVH(xyz.x, xyz.y, xyz.z, surface, dir);
}

Cube::MARK PlayScene::CheckMarkD(int x, int y, int z, SURFACE surface) {
...
Cube::MARK PlayScene::CheckMark(vector<XMINT3> points, SURFACE surface)
{
	/*
	揃ったとき、そのマークを返す
	揃わなければ空白を返す
...
//単一キューブ判定
void PlayScene::CheckMarkSingle(XMINT3 pos, SURFACE surface, WinFlag& flag, FILTER filter = NONE)
{
	//斜め判定
...
void PlayScene::CheckMarkSingle(int x, int y, int z, SURFACE surface, WinFlag& flag, FILTER filter = NONE)
{
	CheckMarkSingle(XMINT3(x, y, z), surface, flag, filter);
}

//回転時の判定
void PlayScene::CheckMarkRotate(XMINT3 pos, DIR dir, WinFlag& flag)
{
	/*
...
void PlayScene::CheckMarkRotate(XMINT3 pos, ROTATE_DIR dir, WinFlag& flag)
{
	switch (dir)
	{
	case ROTATE_DIR::ROT_UP:
...
//斜め判定
void PlayScene::JudgeDiag(XMINT3 pos, Cube::SURFACE surface, WinFlag& flag) {
	//□　　　　上
	//□□□□　前右後左
	//□　　　　下　　　
	switch (surface)
...
//縦横奥判定
void PlayScene::JudgeVHD(XMINT3 pos, Cube::SURFACE surface, WinFlag& flag, FILTER filter)
{
	switch (surface) {
...
void PlayScene::WinProcess(CONTROL winner) {
	this->winner = winner;
	finished = true;
	//カメラ系初期化
	winRotProgress = 0;
...
XMFLOAT3 PlayScene::ConvertPts2Pos(int x, int y, int z) {
	return { (float)x - OUTER_POINT, (float)y - OUTER_POINT, (float)z - OUTER_POINT };
}

XMFLOAT3 PlayScene::Surface2CamRot(SURFACE surface, XMFLOAT3* camRot)
{
	switch (surface)
	{
...
void PlayScene::StoreCamDir(XMFLOAT3 camRotate, CAM_DIR_VT_CARDINAL* camDir_Vertical, CAM_DIR_HN_CARDINAL* camDir_Horizontal)
{
	//カメラの回転情報から移動などで判定に使う方位を登録する
