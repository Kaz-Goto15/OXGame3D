...
private:
	...
	//基本情報
	int hImgBG;
	int hImgTurn[CONTROL_MAX];
	int hImgMode[MODE_MAX];
	int SHOW_TURN_IMAGE_POS_Y;
	int SHOW_MODE_IMAGE_POS_Y;
	void InitImage(int& handle, std::string file, Transform tra);
	...
	struct WinFlag {
		...
		void Init() {
			p1 = false;
			p2 = false;
			p1WinSurface = SURFACE::SURFACE_MAX;
			p2WinSurface = SURFACE::SURFACE_MAX;
		}
	}winFlag;
