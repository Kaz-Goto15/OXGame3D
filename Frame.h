...
public:
    enum MODE {
        AUTO_ASPECT,    //従来の画像クラスと同じ挙動。画面サイズに応じて枠が変形します。Frameクラスのscale.x,yを各方向の画面サイズに対する割合として範囲を指定します。
...
public:
    ...
    /// <param name="posX">X座標指定</param>
    /// <param name="posY">Y座標指定</param>
    void ChangeMode(MODE _mode, float xRatio, float yRatio, float posX, float posY);
};